 ...
 * https://github.com/zanjs/control
 */
(function()
{
	...
	var addWindowItem = function(c, merger, partWith, orderWith, between)
	{
		...
		{
			var ordered = Control.of(part).stratifyWith(orderWith).collect();
		...
		FullJoinPond.prototype.join = function(down, key, lefts, rights)
		{
			if (lefts != null && rights != null)
			{
				for ( var l in lefts)
				{
					...
					{
						if (!down.accept([ key, //
						[ Control.Some(lefts[l]), Control.Some(rights[r]) ] ]))
						{
							return false;
						}
					}
			...
			else if (lefts != null)
			{
				...
				{
					if (!down.accept([ key, //
					[ Control.Some(lefts[l]), Control.None() ] ]))
					{
						return false;
					}
				}
			...
			else if (rights != null)
			{
				...
				{
					if (!down.accept([ key, //
					[ Control.None(), Control.Some(rights[r]) ] ]))
					{
						return false;
					}
				}
		...
		LeftJoinPond.prototype.join = function(down, key, lefts, rights)
		{
			if (lefts != null)
			{
				...
				else
				{
					for ( var l in lefts)
					{
						if (!down.accept([ key, [ lefts[l], Control.None() ] ]))
						{
							return false;
						}
					}
		...
		RightJoinPond.prototype.join = function(down, key, lefts, rights)
		{
			if (rights != null)
			{
				...
				else
				{
					for ( var r in rights)
					{
						if (!down.accept([ key, [ Control.None(), rights[r] ] ]))
						{
							return false;
						}
					}
	...
	function ReduceOp(reducer) // (dat1,dat2) -> dat3
	{
		...
		ReducePond.prototype.get = function()
		{
			...
			? Control.Some(this.settle()) : Control.None();
		};
	...
	function Control()
	{
		...
		this.add = function(op)
		{
			if (op != null)
			{
				return new Control().operator(op).upstream(self).source(
						this.source());
			}
			else
			{
		...
		this.foldByKey = function(zero, folder)
		{
			...
			{
				return Control.of(arr).fold(zero(key), folder);
			});
		};
		...
		this.reduceByKey = function(reducer)
		{
			...
			.mapValues(function(arr, key)
			{
				return Control.of(arr).reduce(reducer).get();
			});
		};
		...
		this.countByValue = function()
		{
			...
			{
				return Control.of(arr).fold(0, function(a, b)
				{
					return a + b;
				});
			}).collectAsMap();
		...
		{
			var arr = this.take(1);
			return arr.length > 0 ? Control.Some(arr[0]) : Control.None();
		};
	...
	Control.eod = function()
	{
		return endOfData;
	};

	Control.mapOfPairs = function(pairs)
	{
		var map = {};
	...
	Control.pairsOfMap = function(map)
	{
		var pairs = [];
	...
	Control.of = function(data)
	{
		if (data instanceof Array)
		{
			return new Control().source(new Source(data, arguments[1],
					arguments[2]));
		}
		else if (data instanceof Function)
		{
			return new Control().source(new Spring(data, arguments[1]));
		}
		else
		{
			return Control.of(Control.pairsOfMap(data));
		}
	};
	...
	Control.Some = function(val)
	{
		return new Some(val);
	};

	Control.None = function()
	{
		return new None();
	};
